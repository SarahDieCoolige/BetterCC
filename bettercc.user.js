        ...
        if (theme >= 1){
          ...
          setTimeout(function () {
            ...
            //bgInterval = setInterval(function () { $(iframeWindow).find("body").css('background-color', 'eba096').css('color', 'black'); }, 500);
             bgInterval = setInterval(function(){iframe.contentWindow.setbgcol("c8dae0", "black"); } , 300);
          }, 1000);
        }
        if(theme == 2 ) {
          setTimeout(function () {
            $(iframeWindow).find('#darkmode').remove();
            $(iframeWindow).find('head').append('<style id="darkmode">' + dark_mode_css + '</style>');
            if(bgInterval) clearInterval(bgInterval);
            bgInterval = setInterval(function(){iframe.contentWindow.setbgcol("0a1822", "c8dae0"); } , 300);
          }, 1000);
        }
      } else {
